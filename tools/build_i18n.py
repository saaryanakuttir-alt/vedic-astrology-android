...
def build_language(code):
    ...
    months = list(getattr(src, "MONTHS", ())) + list(getattr(src, "MONTHS_LONG", ()))
    values = list(table.values()) + [getattr(src, "NAME", "")] + months
    ...
    months_enc = tuple(encode(m) for m in getattr(src, "MONTHS", ()))
    months_long_enc = tuple(encode(m) for m in getattr(src, "MONTHS_LONG", ()))
    if problems:
...
if __name__ == "__main__":
    for code in LANGS:
        if os.path.exists(os.path.join(SRC, f"{code}.py")):        # only the languages that have a source file yet
            build_language(code)
